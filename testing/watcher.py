...
from project_library_layer.initializer.initializer import watcher_input_file_path, get_project_id
...
def create_directory_input_file_storage():
        ...
        if container == 'machine-learning-327030':
            ...
            if event_type == 'Microsoft.Storage.BlobCreated':
                ...
                if len(file_names) > 0:
                    ...
                    if not project_id:
                        watcher_data.update({'status':'False','message':"Project id {} is not proper".format(project_id)})
                        insert_watcher_event_into_db(watcher_data)
                        return False
                    ...
                    for file in file_names:
                        ...
                        if not result['status']:
                            ...
                            insert_watcher_event_into_db(watcher_data)
                            continue
                        watcher_data.update({'status':True,'message':'Recevied file {} is copied into directory {} '.format(file,file_path_location)})
...
async def start_watcher():
    ...
    checkpoint_store = BlobCheckpointStore.from_connection_string(conn_str="DefaultEndpointsProtocol=https;AccountName=storageaccountrgai1a757;AccountKey=j4TYtRYwIDyUNL4lKXXIfYgCftrTbJPhPHC8i/j66XSf6s81ruCaQqDnjh5WDMjrqqc4hulvZ8wxcYQb8KfukA==;EndpointSuffix=core.windows.net",
